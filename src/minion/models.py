"""SQLAlchemy persistence schema for durable control-plane state.

Only information required to recover/replay task execution belongs here. Repository
indexes and dependency caches are derived data and intentionally remain rebuildable.
"""
...
class Base(DeclarativeBase):
    """Declarative metadata root used by Alembic and local bootstrap."""


class TaskRow(Base):
    """One logical engineering request with optimistic versioning."""
...
class SessionRow(Base):
    """Compact durable working memory for one coding-agent session."""
    ...
    current_plan: Mapped[list[str]] = mapped_column(
        JSON, default=list
    )
    active_constraints: Mapped[list[str]] = mapped_column(
        JSON, default=list
    )
    active_skills: Mapped[list[str]] = mapped_column(
        JSON, default=list
    )
...
class EventRow(Base):
    """Append-only ordered evidence for replay, recovery and audit."""
...
class EnvironmentLeaseRow(Base):
    """Control-plane record for the replaceable sandbox lease."""
